...
const CartItem = ({ newsItem }) => {
    ...
    return (
        <Link to={`${newsItem?.source?.id}`} className='border-r border-b border-l border-gray-400 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r flex flex-col gap-5 w-full md:w-[438px] h-auto p-3 md:p-0 md:h-[520px]'>
            ...
            <div className='px-3'>
                <div className='mb-2'>
                    <h3 className='text-gray-900 font-bold text-lg mb-5 hover:text-indigo-600 w-[388px] h-[45px] inline-block'>
                        {newsItem?.title.length > 10 ? newsItem?.title.slice(0,90): newsItem?.title || 'Title not available'}
                    </h3>
                    <p className='text-gray-700 text-sm text-justify w-[388px] h-[106px]'>
                        {newsItem?.description || 'Description not available'}
                    </p>
                    <div className='flex gap-3 items-center mt-1'>
                        <p className='text-[#2A2A2A] w-24 h-5 text-[13px]'>{new Date(newsItem?.publishedAt).toLocaleTimeString()}</p>
